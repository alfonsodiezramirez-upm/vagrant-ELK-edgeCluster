...
#Es importante no ejecutar este comando como root dado que si ejecutamos la creación del token como tal, el fichero service_tokens no cuenta con permisos de lectura para el usuario elasticsearch y por tanto no levanta el servicio, como alternativa si se muestra el error /etc/default/elasticsearch: permission denied, podemos ejecutarlo como root y cambiar los permisos de service_tokens más adelante.
